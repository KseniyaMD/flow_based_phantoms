...
# Concatenation before affine net
...
class HierarchyCoupling(nn.Module):
    def __init__(self, in_channel, out_channel, weight_type='fixed', seq_dim=3):
        ...
        self.param_embed = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(32, in_channel)  # чтобы получить по-канальный коэффициент
        )
    ...
    def forward(self, input, seq_params):
        ...
        seq_base = seq_params.mean(dim=-1)

        # embed seq params для каждого канала
        seq_feat = self.param_embed(seq_base)  # [B, in_channel]
        seq_feat = seq_feat.unsqueeze(-1).unsqueeze(-1).expand(b, c, h, w)

        # combine images and seq info
        input_cond = torch.cat([input, seq_feat], dim=1)
        feature = self.affine_net(input_cond)
