...
class SeqParamsGenerator(nn.Module):
    def __init__(self, seq_param_dim, feature_dim):
        super().__init__()
        # feature_dim = количество выходных каналов, соответствующих каждому входному каналу
        self.net = nn.Sequential(
            nn.Linear(seq_param_dim, feature_dim * 2),  # γ и β по одному каналу
            nn.ReLU(),
            nn.Linear(feature_dim * 2, feature_dim * 2)
        )

    def forward(self, seq_params):
        """
        seq_params: [batch, num_channels, num_sequence_params]
        """
        # flatten для обработки каждого канала отдельно
        batch, num_ch, num_seq = seq_params.shape

        # reshape → (batch*num_channels, num_sequence_params)
        seq_flat = seq_params.view(batch * num_ch, num_seq)

        # прогоняем через MLP
        out = self.net(seq_flat)  # (batch*num_channels, 2*feature_dim)

        # разделяем γ и β
        gamma, beta = out.chunk(2, dim=1)  # оба (batch*num_channels, feature_dim)

        # обратно в (batch, num_channels, feature_dim)
        gamma = gamma.view(batch, num_ch, -1)
        beta  = beta.view(batch, num_ch, -1)

        return gamma, beta


class AffineFiLMNet(nn.Module):
    def __init__(self, in_channel, out_channel, seq_param_dim):
        super().__init__()

        # твой обычный свёрточный блок
        self.conv_net = nn.Sequential(
            nn.Conv2d(in_channels=in_channel, out_channels=in_channel * 2, kernel_size=3, padding=1),
            nn.InstanceNorm2d(in_channel * 2),
            nn.ReLU(inplace=True),

            nn.Conv2d(in_channels=in_channel * 2, out_channels=in_channel * 2, kernel_size=3, padding=1),
            nn.InstanceNorm2d(in_channel * 2),
            nn.ReLU(inplace=True),

            nn.Conv2d(in_channels=in_channel * 2, out_channels=out_channel, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
        )

        # FiLM генератор для параметров
        self.film_gen = SeqParamsGenerator(seq_param_dim, out_channel)

    def forward(self, x, seq_params):
        # x: [batch, in_channel, H, W]
        # seq_params: [batch, in_channel, seq_param_dim]

        features = self.conv_net(x)  # [batch, out_channel, H, W]

        # выдаёт γ и β
        gamma, beta = self.film_gen(seq_params)
        # gamma, beta: [batch, in_channel, out_channel_per_channel_embedding]

        gamma = gamma.mean(dim=1, keepdim=True)  # -> [batch, 1, out_channel]
        beta = beta.mean(dim=1, keepdim=True)

        # убираем размерность канала-1 → [batch, out_channel]
        gamma = gamma.squeeze(1)
        beta = beta.squeeze(1)

        # добавляем spatial dims
        gamma = gamma.unsqueeze(-1).unsqueeze(-1)  # [batch, out_channel, 1, 1]
        beta = beta.unsqueeze(-1).unsqueeze(-1)  # [batch, out_channel, 1, 1]
...
class HierarchyCoupling(nn.Module):
    def __init__(self, in_channel, out_channel, weight_type='fixed'):
        ...
        self.affine_net = AffineFiLMNet(in_channel=self.in_channel, out_channel=self.out_channel, seq_param_dim=3) # seq_param_dim=(self.in_channel, 3)
        self.adain = AdaIN()
        self.style_mlp = MLP(8, self.out_channel * 2, self.out_channel * 3, 3, norm='none', activ='relu')
    ...
    def forward(self, input, in_seq_params):
        b_size, n_channel, height, width = input.shape

        feature = self.affine_net(input, in_seq_params)
        self.feat = feature
