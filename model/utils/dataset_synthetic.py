...
class BaseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        imgA_only_resized = self.transform(imgA_only_resized)

        if not self.return_name:
            return imgA_only_resized, imgB, name
        else:
            return imgA, imgB, '{}_to_{}'.format(pathA.split('/')[-1].split('.')[0], pathB.split('/')[-1].split('.')[0]), imgA_not_normalized, imgA_max_vals, imgA_only_resized, name
